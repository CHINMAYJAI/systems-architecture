class LookAlgorithm :
    ...
    def lookSchedulingAlgorithm(self):
        """Main scan scheduling algorithm calculation function"""

        # asking how many address must be there in the request queue excluding minimum and maximum cylindrical value
        number_of_addresses = int(input("Irrespective of minimum cylinder value and maximum cylinder value\nHow many address do you want to enter?\n"))
        ...
        while True:
            ...
            if (self.direction.lower())[0] == "r":
                ...
                for i in range(pointer_index,len(self.request_queue)-1):
                ...
                for i in range(pointer_index-1,0,-1):
                    number = self.request_queue[i]
                    local_array.append(number)
                    if len(local_array) == 2:
                        difference = abs(local_array[0] - local_array[1])
            ...
            # movement of left side
                ...
                for i in range(pointer_index,0,-1):
                    number= self.request_queue[i]
                    local_array.append(number)
                    if len(local_array) == 2:
                        difference = abs(local_array[0] - local_array[1])
                        ...
                        local_array.pop(0)
                # now calculating the another half
                for i in range(len(self.request_queue)-2,pointer_index,-1):
                    number = self.request_queue[i]
                    local_array.append(number)
                    if len(local_array) == 2:
                        difference = abs(local_array[0] - local_array[1])
                ...
                continue
        # performing the summation of all the numbers that are present inside the number_array
        print(number_array)
        self.seek_time = sum(number_array)

    def main(self):
        """Calling function"""
        self.lookSchedulingAlgorithm()
...
if __name__ == "__main__" :
    # class object
    look_algo = LookAlgorithm()
    # displaying the seek time
